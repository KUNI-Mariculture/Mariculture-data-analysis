...
calc_scores <- function(dat) {
  ...
  #nutrition has to be normalized
  out$mean_nutrition <- (out$mean_nutrition-min(out$mean_nutrition,na.rm=T))/(max(out$mean_nutrition,na.rm=T)-min(out$mean_nutrition,na.rm=T))
...
calc_diffs <- function(dat.full,dat.reduced,title) {
  ...
  hist(reliancediff,main="Reliance Change",xlim=c(-0.5,0.5))
  hist(oppdiff,main="TotalOpp Change",xlim=c(-0.5,0.5))
  title(title,outer=T)
}
...
for(i in 1:length(testvars)) {
  ...
  png(filename)
  dat.new[-i] %>% calc_scores(.) %>% calc_diffs(dat.norm,.,title=paste(testvars[i], "removed"))
  dev.off()
}

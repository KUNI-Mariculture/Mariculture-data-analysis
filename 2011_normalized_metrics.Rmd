...
dat <- dat %>%
  select(country,year,
         Q.balance,UV.balance,energy_adequacy,undernourish,
...
dat.norm <- dat2 %>% distinct(country)
```
...
chance that a random person from the population will be undernourished, scaled relative to maximum in the data.

```{r undernourished}
# Character vector right now (remove "<5.0")
dat.norm$undernourish <- sapply(dat.norm$undernourish,function(x) gsub("<","",x)) %>% as.numeric()
# Use - undernourish to switch direction of scale (so least undernourished gets closer to 1)
dat.norm <- mutate(dat.norm,undernourish=normalize(-undernourish))
hist(dat.norm$undernourish,xlab="Normalized Undernourished Score",main="")
```
...
dat.norm$trade <- dat.norm %>% select(Q.balance,UV.balance,prod_ratio) %>% apply(.,1,FUN=mean)
dat.norm$fs <- dat.norm %>% select(energy_adequacy,gdp,undernourish) %>% apply(.,1,FUN=mean)
dat.norm$nutrition <- dat.norm %>% select(polyunsatFA_percentseafood:zinc_percentseafood) %>% apply(.,1,FUN=mean)
...
```{r output}
write.csv(dat.norm,file=paste0(W_D,"/data/data_normalized_101116.csv"))
```